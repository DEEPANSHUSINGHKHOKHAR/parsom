...
async function listDeletedCoupons(req, res, next) {
  try {
    const data = await couponsService.listDeletedCoupons();
    res.status(200).json({
      success: true,
      message: 'Deleted coupons fetched successfully.',
      data,
    });
  } catch (error) {
    next(error);
...
async function restoreCoupon(req, res, next) {
  try {
    const data = await couponsService.restoreCoupon(req.params.couponId);
    await writeAuditLog({
      actorType: 'admin',
      actorId: req.user.id,
      actionKey: 'coupons.restore',
      resourceType: 'coupon',
      resourceId: data.couponId,
      req,
    });
...
async function permanentlyDeleteCoupon(req, res, next) {
  try {
    const data = await couponsService.permanentlyDeleteCoupon(req.params.couponId);
    await writeAuditLog({
      actorType: 'admin',
      actorId: req.user.id,
      actionKey: 'coupons.permanent_delete',
      resourceType: 'coupon',
      resourceId: data.couponId,
      req,
    });
