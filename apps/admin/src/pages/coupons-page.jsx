...
export default function CouponsPage() {
  ...
  const usageSummary = items.reduce(
    (summary, item) => ({
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[8px] border border-[#171412]/10 bg-[#fffaf4] p-5">
          <p className="text-xs uppercase text-[#756c63]">Discount Given</p>
          <p className="mt-2 text-3xl font-semibold text-[#171412]">
            {new Intl.NumberFormat('en-IN', {
              style: 'currency',
              currency: 'INR',
              maximumFractionDigits: 0,
            }).format(usageSummary.discountGiven)}
          </p>
        </div>
        <div className="rounded-[8px] border border-[#171412]/10 bg-[#fffaf4] p-5">
                ...
                {item.usageLimit ? (
                  <div className="mt-3 h-2 w-64 max-w-full overflow-hidden rounded-full bg-[#e5ded5]">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          ((item.orderUsageCount ?? item.totalUsed ?? 0) /
                            item.usageLimit) *
                            100,
                          100
                        )}%`,
